...
class VerifyToken():
    ...
    def __init__(self, token, permissions=None, scopes=None):
        self.token = token
        # sanity check, let's split at . and decode the (should be) 3 parts
        # if not, we have a problem
        try:
            print(token)
            header, payload, signature = self.token.split('.')
            header = jwt.utils.base64url_decode(header)
            payload = jwt.utils.base64url_decode(payload)
            signature = jwt.utils.base64url_decode(signature)
        except:
